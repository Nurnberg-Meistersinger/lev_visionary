...
def run():
    print("\n=== Запуск мониторинга TechCrunch Startups ===")

    # Фетчим больше статей для ранжирования (например, 15)
    items = fetch_latest_articles(limit=15)
    ...
    new_articles = []
    items_map = {}  # slug -> item для быстрого поиска
    ...
    # 4) Формируем дайджест-пост
    digest_lines = ["📰 <b>TechCrunch Startups Digest</b>\n"]
    ...
    digest_lines.append("\n#techcrunch #startups #digest")

    digest_message = "\n".join(digest_lines)

    # 5) Отправляем дайджест в канал
    try:
    ...
    # 6) Отправляем комментарии к каждой статье в Discussion Group
    ...
    # 7) Сохраняем обработанные slugs
